...
pub struct LTCState<B: Backend> {
    ...
    pub h_state_v: Tensor<B, 2>,
    pub h_state_n: Tensor<B, 2>,
    // pub c_state: Option<Tensor<B, 2>>,
    pub c_state_v: Option<Tensor<B, 2>>,
    pub c_state_n: Option<Tensor<B, 2>>,
}
...
pub struct LTC<B: Backend> {
    ...
    lstm_v: Option<LSTMCell<B>>,
    lstm_n: Option<LSTMCell<B>>,
    state_size: usize,
    sensory_size: usize,
    motor_size: usize,
...
impl<B: Backend> LTC<B> {
    pub fn new(
        ...
        let sensory_size = wiring.input_size;
        let motor_size = wiring.output_size;
        ...
        //     None
        // };
        let lstm_v = if config.mixed_memory {
            let lstm_config = LSTMCellConfig {
                input_size: config.input_size,
                hidden_size: state_size,
                dropout: 0.0,
        ...
        let lstm_n = if config.mixed_memory {
            let lstm_config = LSTMCellConfig {
                input_size: config.input_size,
                hidden_size: state_size,
                dropout: 0.0,
    ...
    pub fn init_state(&self, batch_size: usize, device: &B::Device) -> LTCState<B> {
        ...
        let h_state_v = Tensor::zeros([batch_size, self.state_size], device);
        let h_state_n = Tensor::zeros([batch_size, self.state_size], device);
        // let c_state = if self.mixed_memory {
        //     Some(Tensor::zeros([batch_size, self.state_size], device))
        // } else {
        //     None
        // };
        let c_state_v = if self.mixed_memory {
            Some(Tensor::zeros([batch_size, self.state_size], device))
        } else {
            None
        };
        let c_state_n = if self.mixed_memory {
            Some(Tensor::zeros([batch_size, self.state_size], device))
        } else {
            None
        };
        // LTCState { h_state, c_state }
        LTCState { h_state_v, h_state_n, c_state_v, c_state_n }
    }
    ...
    pub fn forward(
        ...
        for t in 0..seq_len {
            ...
            if self.mixed_memory {
                ...
                // }
                if let Some(lstm) = &self.lstm_v {
                    let lstm_state = LSTMState {
                        hidden: current_state.h_state_v.clone(),
                        cell: current_state.c_state_v
                            .clone()
                            .expect("LSTM cell state should exist when mixed_memory is true"),
                    };
                    
                    let (new_hidden, new_state) = lstm.forward(timestep_input.clone(), lstm_state);
                    current_state.h_state_v = new_hidden;
                    current_state.c_state_v = Some(new_state.cell);
                }
                if let Some(lstm) = &self.lstm_n {
                    let lstm_state = LSTMState {
                        hidden: current_state.h_state_n.clone(),
                        cell: current_state.c_state_n
                            .clone()
                            .expect("LSTM cell state should exist when mixed_memory is true"),
                    };
                    
                    let (new_hidden, new_state) = lstm.forward(timestep_input.clone(), lstm_state);
                    current_state.h_state_n = new_hidden;
                    current_state.c_state_n = Some(new_state.cell);
                }
            }
            ...
            // );
            let (h_out, (h_state_v, h_state_n)) = self.rnn_cell.forward(
                timestep_input,
                (current_state.h_state_v, current_state.h_state_n),
                elapsed_time,
                Some(self.rnn_cell.tolerance),
            );
    
            // current_state.h_state = h_state;
            current_state.h_state_v = h_state_v;
            current_state.h_state_n = h_state_n;
