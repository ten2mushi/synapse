...
use synapse::{
    ...
    wiring::v2::{Wiring, LayerConfig, Fanout}
};
...
impl<B: Backend> WaveModel<B> {
    pub fn new(wiring: Wiring<B>, _num_waves: usize, hidden_size: usize, ltc_config: LTCConfig, model_name: &str, uuid1: &str, device: &B::Device) -> Self {

        let filename = format!("./output/{}_{}_{}/{}_{}_{}_{}.png", model_name,ltc_config.seed, uuid1, "NCPS_V2".to_string(), hidden_size, ltc_config.clone().integration_method, ltc_config.clone().ode_unfolds);
...
pub fn train<B: AutodiffBackend>(wiring: Wiring<B>, config: TrainingConfig, ltc_config: LTCConfig, dataset_config: DatasetConfig, device: B::Device) {
...
fn main() {
    ...
    let layer_configs = vec![
        ...
        LayerConfig {
            ...
            connections: vec![
                Fanout::Forward(0.9, 0.8),    // Connect to next layer with 30% sparsity, 70% excitatory
        ...
        LayerConfig {
            num_units: 7,
            connections: vec![
                Fanout::Forward(0.9, 0.8),    // Connect to next layer
                // Fanout::Backward(0.3, 0.5),   // Connect back to previous hidden layer
                // Fanout::Recurrent(3, 0.4),    // 3 recurrent connections
            ],
        },
        ...
        LayerConfig {
            num_units: 4,
            connections: vec![
                Fanout::Forward(0.9, 0.8),   // Connect back to previous layer
                // Fanout::Recurrent(1, 0.5),    // 1 recurrent connection
            ],
        },
        ...
        LayerConfig {
            ...
            connections: vec![
                // Fanout::Backward(0.1, 0.8),   // Connect back to previous layer
                // Fanout::Recurrent(1, 0.5),    // 1 recurrent connection
            ],
        },
    ];
    ...
    // Create the wiring
    // let wiring: Wiring<LibTorch> = Wiring::new(
    ...
    // let mut wiring = WiringImpl::AutoNCP(
    //     AutoNCP::new(
    ...
    // let mut wiring = WiringImpl::FullyConnected(
    //     FullyConnected::new(
    //         hidden_size,
    //         Some(1), // output dim
    //         true,
